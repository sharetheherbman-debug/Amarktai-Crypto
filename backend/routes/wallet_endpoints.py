...
async def currency_converter(data: dict, user_id: str = Depends(get_current_user)):
    ...
    try:
        ...
        if from_cur not in SUPPORTED:
            raise HTTPException(status_code=400, detail=f"Unsupported from_currency '{from_cur}'. Supported: {', '.join(sorted(SUPPORTED))}")
        if to_cur not in SUPPORTED:
            raise HTTPException(status_code=400, detail=f"Unsupported to_currency '{to_cur}'. Supported: {', '.join(sorted(SUPPORTED))}")
