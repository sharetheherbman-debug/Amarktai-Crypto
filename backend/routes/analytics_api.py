...
# ============================================================================
# COMPATIBILITY ENDPOINTS - Frontend expects these exact paths
# ============================================================================

@router.get("/performance")
async def get_performance_compat(
    period: str = Query("all", regex="^(today|7d|30d|all)$"),
    user_id: str = Depends(get_current_user)
):
    """
    Compatibility endpoint for GET /api/analytics/performance
    
    Frontend expects: { total_trades, win_rate, ... }
    This is an alias to /performance_summary with compatible response format
    """
    logger.warning("COMPAT: /api/analytics/performance called - migrate to /performance_summary")
    ...
    try:
        ...
        logger.error(f"Performance compat endpoint error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
