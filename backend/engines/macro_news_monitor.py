...
class MacroEvent:
    ...
    is_simulated: bool = False  # True when created by simulate_events() — not real market data
...
class MacroNewsMonitor:
    ...
    async def add_event(
        ...
        Args:
            ...
            is_simulated: Set True when this event is generated by simulate_events()
                          or any source other than a real economic data feed.
        ...
        logger.info(
            ...
            f"(risk adjustment: {risk_adjustment:+.2f}, simulated={is_simulated})"
        )
    ...
    async def get_macro_signal(self) -> Optional['MacroSignal']:
        """
        Get current macro signal for portfolio adjustment.

        Returns None when ENABLE_LIVE_TRADING=true and no real events have been
        fed via ``add_event()`` — this prevents simulated events from affecting
        live position sizing.

        Returns:
            MacroSignal or None (when simulated events should be excluded)
        """
        import os
        ...
        # In live mode, only use events explicitly added via add_event with is_simulated=False
        if live_trading_active:
            real_events = [e for e in recent_events if not e.is_simulated]
            if not real_events:
                logger.debug(
                    "MacroNewsMonitor: no real macro events available — "
                    "returning None (macro signal suppressed in live trading)"
                )
                return None
