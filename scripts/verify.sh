...
test_endpoint "GET" "/api/diagnostics/reserves" "Reserves diagnostics" "200|401"
test_endpoint "GET" "/api/diagnostics/email-status" "Email service status" "200"
echo ""

echo "8. Wallet Transfer Gating (should require auth)"
echo "------------------------------------------------"
echo "Testing that transfers are properly gated..."
# This should fail without auth (401) or require 2FA/approval
test_endpoint "POST" "/api/wallet/transfers/create" "Transfer creation requires auth" "401|422"
test_endpoint "GET" "/api/wallet/transfers" "Transfer list requires auth" "200|401"
echo ""

echo "9. Real-time Features"
echo "---------------------"
